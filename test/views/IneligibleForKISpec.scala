...
class IneligibleForKISpec extends UnitSpec with WithFakeApplication with MockitoSugar with FakeRequestHelper {
  ...
  "The Ineligible for Knowledge Intensive page" should {
    ...
    "Verify that the Ineligible for Knowledge Intensive page contains the correct elements "  in new SetupPage {
    ...
    //TODO Unit test cannot be completed as the backlinks aren't set up currently. Needs to be linked to 10 year plan, Operating Costs

    "Verify that the Ineligible for Knowledge Intensive page contains the correct elements without a back link "  in new SetupPage {
      val document: Document = {
        val userId = s"user-${UUID.randomUUID}"
