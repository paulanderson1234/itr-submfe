...
class RegisteredAddressSpec extends ViewTestSpec {
  ...
  "The Registered Address page" should {
    ...
    "Verify that Registered Address page contains the correct elements " +
      "when a valid RegisteredAddressModel is passed as returned from keystore" in new Setup {
    ...
    "Verify that Registered Address page contains the correct elements " +
      "when no data is returned from keystore" in new Setup {
      val document: Document = {
        setupMocks()
        val result = TestController.show.apply(authorisedFakeRequest)
        Jsoup.parse(contentAsString(result))
    ...
    "Verify that the contact address contains the correct elements and error when an invalid model is posted" in new Setup {
      val document: Document = {
        setupMocks()
