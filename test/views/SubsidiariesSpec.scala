...
class SubsidiariesSpec extends UnitSpec with WithFakeApplication with MockitoSugar{

  val mockKeystoreConnector = mock[KeystoreConnector]

  val SubsidiariesModel = new SubsidiariesModel("Yes")
  ...
  "Verify that the Subsidiaries page contains the correct elements " +
    "when a valid SubsidiariesModel is passed as returned from keystore" in new SetupPage {
    val document : Document = {
      val userId = s"user-${UUID.randomUUID}"
      when(mockKeystoreConnector.fetchAndGetFormData[SubsidiariesModel](Matchers.any())(Matchers.any(), Matchers.any()))
  ...
  "Verify that Subsidiaries page contains show the error summary when an invalid model (no radio button selection) is submitted" in new SetupPage {
    val document : Document = {
      val userId = s"user-${UUID.randomUUID}"
