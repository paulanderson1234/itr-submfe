...
class TradeStartDateControllerSpec extends BaseSpec {
  ...
  def setupShowMocks(tradeStartDateModel: Option[TradeStartDateModel] = None, processingModel:
                Option[EisSeisProcessingModel] = None): Unit = {
    when(mockS4lConnector.fetchAndGetFormData[TradeStartDateModel](Matchers.eq(KeystoreKeys.tradeStartDate))(Matchers.any(),
      Matchers.any(), Matchers.any())).thenReturn(Future.successful(tradeStartDateModel))
  ...
  def setupSubmitMocks(result: Option[Boolean],processingModel:
  Option[EisSeisProcessingModel] = None): Unit = {
  ...
  "Sending a valid Yes form submission to the TradeStartDateController when authenticated and enrolled" should {
      "redirect to the INTERNAL_SERVER_ERROR page as the API call has failed" in {
        val formInput = Seq(
          ...
          "tradeStartMonth" -> "11",
          "tradeStartYear" -> "2014")
      setupSubmitMocks(None, Some(eisSeisProcessingModelEligible))
      mockEnrolledRequest(eisSeisSchemeTypesModel)
      submitWithSessionAndAuth(TestController.submit,formInput:_*)(
        result => {
