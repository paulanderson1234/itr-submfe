...
class AttachmentsAcknowledgementControllerSpec extends BaseSpec {
  ...
  "Sending an Authenticated and Enrolled GET request with a session to AttachmentsAcknowledgementController for combined" should {
    "return a 200, send an email and delete the current application when a valid submission data is submitted with minimum data" in new SetupPageMinimum {
      when(mockS4lConnector.clearCache()(Matchers.any(),Matchers.any())).thenReturn(HttpResponse(NO_CONTENT))
      setupMocks()
      mockEnrolledRequest(eisSeisSchemeTypesModel)
      val result = TestController.show.apply(authorisedFakeRequest)
  ...
  "Sending a POST request to the Attachments Acknowledgement Controller for combined when authenticated and enrolled" should {
