...
trait LifetimeLimitHelper {
  ...
  val previousSchemesFut = ControllerHelpers.getAllInvestmentFromKeystore(keyStoreConnector)


  def previousSchemesAmount(): Future[Int] = previousSchemesFut.map(previousSchemes => previousSchemes.foldLeft(0)(_ + _.investmentAmount))

  def exceedsLifetimeLogic(isKi: Boolean): Future[Boolean] = {
