...
object PreviousInvestmentsAllowanceExceededController extends PreviousInvestmentsAllowanceExceededController {
  ...
  def show: Action[AnyContent] = featureSwitch(applicationConfig.seisFlowEnabled) {
    AuthorisedAndEnrolled.async { implicit user => implicit request =>
      s4lConnector.fetchAndGetFormData[SchemeTypesModel](KeystoreKeys.selectedSchemes).map {
        ...
        case e: Exception => Logger.warn(s"[PreviousInvestmentsAllowanceExceededController][show] Error calling fetchAndGetFormData: ${e.getMessage}")
