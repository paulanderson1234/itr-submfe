...
trait WhoCanUseNewServiceController extends FrontendController with AuthorisedAndEnrolledForTAVC {

  override val acceptedFlows = Seq()

  val show = AuthorisedAndEnrolled.async { implicit user =>
    implicit request =>
      Future.successful(Ok(WhoCanUseNewService()))
  }

  val submit = AuthorisedAndEnrolled.async { implicit user =>
    implicit request =>
      Future.successful(Redirect(controllers.hubGuidance.routes.HubGuidanceFeedbackController.show()))
  }
}